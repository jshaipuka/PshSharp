...
{
    protected internal BooleanStack _boolStack = new();
    protected internal ObjectStack _codeStack = new();

    protected internal long _evaluationExecutions;
    protected internal ObjectStack _execStack = new();
    protected internal FloatStack _floatStack = new();

    public Dictionary<string, AtomGenerator> _generators = new();

    // XXX What is the input stack and pusher really for?
    protected internal InputPusher _inputPusher = new();
    protected internal ObjectStack _inputStack = new();

    public Dictionary<string, Instruction> _instructions = new();

    protected internal IntStack _intStack = new();
    protected internal GenericStack<string> _nameStack = new();

    // This kind of looks like it wants to be a map, but I think the order is more important.
    protected internal List<Tuple<string, Stack>> _stacks = new();
    ...
    public RandomProgram randProgram = new();
    ...
    public Interpreter()
    {
        ...
        */
        ...
        _generators["float.erc"] = randFloat = new FloatAtomGenerator();
        _generators["integer.erc"] = randInt = new IntAtomGenerator();
        _generators["boolean.erc"] = randBool = new BoolAtomGenerator();

        DefineInstruction("integer.-", (int a, int b) => unchecked(a - b));
        // DefineInstruction("integer./", (int a, int b) => { return (b != 0 ? a / b : 0); } );
        DefineInstruction("integer./", (int a, int b) => b != 0 ? a / b : 0);
        ...
        DefineInstruction("float.+", (float a, float b) => a + b);
        DefineInstruction("float.-", (float a, float b) => a - b);
        DefineInstruction("float./", (float a, float b) => b != 0f ? a / b : 0f);
        DefineInstruction("float.%", (float a, float b) => b != 0f ? a % b : 0f);
        DefineInstruction("float.*", (float a, float b) => a * b);
        DefineInstruction("float.pow", (float a, float b) => (float)Math.Pow(a, b));
        DefineInstruction("float.log", (float a, float b) => (float)Math.Log(a, b));
        DefineInstruction("float.=", (float a, float b) => a == b);
        DefineInstruction("float.>", (float a, float b) => a > b);
        DefineInstruction("float.<", (float a, float b) => a < b);
        DefineInstruction("float.min", (float a, float b) => a < b ? a : b);
        DefineInstruction("float.max", (float a, float b) => a > b ? a : b);
        DefineInstruction("float.sin", (float a) => (float)Math.Sin(a));
        DefineInstruction("float.cos", (float a) => (float)Math.Cos(a));
        DefineInstruction("float.tan", (float a) => (float)Math.Tan(a));
        DefineInstruction("float.exp", (float a) => (float)Math.Exp(a));
        DefineInstruction("float.abs", (float a) => a < 0 ? -a : a);
        DefineInstruction("float.neg", (float a) => -a);
        DefineInstruction("float.ln", (float a) => (float)Math.Log(a));
    ...
    public void DefineInstruction(string inName, Instruction inInstruction)
    {
        // inName = InstructionCase(inName);
        _instructions[inName] = inInstruction;
        _generators[inName] = new InstructionAtomGenerator(inName);
    }
    ...
    public void DefineEnumConstants(string prefix, Type enumType)
    {
        foreach (var name in Enum.GetNames(enumType))
            DefineConstant(prefix + "." + name.ToLower(), (int)Enum.Parse(enumType, name));
    }
    ...
    public void ClearStacks()
    {
        ...
        foreach (var t in _stacks) t.Item2.Clear();
    }
    ...
    {
        internal readonly string _instruction;
    ...
    public class FloatAtomGenerator : RandAtomGenerator<float>
    {
        ...
        {
            var r = (float)Rng.NextDouble() * (max - min);
            if (resolution > 0f)
                r -= r % resolution;
            return r + min;
        }
    ...
    public class IntAtomGenerator : RandAtomGenerator<int>
    {
