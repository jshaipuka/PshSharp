...
internal class Quote : Instruction
{
    ...
    {
        var cstack = inI.CodeStack();
        var estack = inI.ExecStack();
        if (estack.Size() > 0) cstack.Push(estack.Pop());
    }
}
...
internal class Pop : StackInstruction
{
    ...
    {
        if (_stack.Size() > 0) _stack.Popdiscard();
    }
}
