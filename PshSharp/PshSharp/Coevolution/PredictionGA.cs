...
public abstract class PredictionGA : GA
{
    ...
    protected internal override void BeginGeneration()
    {
        if (_generationCount % _generationsBetweenTrainers == _generationsBetweenTrainers - 1)
        {
            ...
            var newTrainer = (PushGPIndividual)ChooseNewTrainer().Clone();
    ...
    {
        var individualVariances = new List<float>();
        for (var i = 0; i < _solutionGA.GetPopulationSize(); i++)
        {
            var individual = (PushGPIndividual)_solutionGA.GetIndividualFromPopulation(i);
            var predictions = new List<float>();
            for (var j = 0; j < _populations[_currentPopulation].Length; j++)
            {
                var predictor = (PredictionGAIndividual)_populations[_currentPopulation][j];
        ...
        var highestVarianceIndividual = 0;
        var highestVariance = individualVariances[0];
        for (var i_1 = 0; i_1 < _solutionGA.GetPopulationSize(); i_1++)
    ...
    protected internal virtual PredictionGAIndividual GetBestPredictor()
    {
        ...
        var bestPredictor = _populations[_currentPopulation][0];
        foreach (var ind in _populations[_currentPopulation])
    ...
    private void InitTrainerPopulation()
    {
        ...
        var individual = new PushGPIndividual();
    ...
    {
        var sampleMean = SampleMean(list);
        float sum = 0;
        foreach (var element in list) sum += (element - sampleMean) * (element - sampleMean);
